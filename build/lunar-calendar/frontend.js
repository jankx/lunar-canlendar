/******/ (() => { // webpackBootstrap
/*!*******************************************!*\
  !*** ./blocks/lunar-calendar/frontend.js ***!
  \*******************************************/
(function () {
  'use strict';
  ...
  ready(function () {
    ...
    function getLunarDate(mDate) {
      try {
        if (window._calendar && window._calendar.SolarDate) {
          ...
          const y = mDate.year();
          const solar = new window._calendar.SolarDate({
            day: d,
            month: m,
            year: y
          });
          ...
          return {
            day: lunar.day,
            month: lunar.month,
            yearName: lunar.getYearName ? lunar.getYearName() : '',
            dayName: lunar.getDayName ? lunar.getDayName() : '',
            monthName: lunar.getMonthName ? lunar.getMonthName() : ''
          };
        }
      } catch (e) {
        console.warn('lunar calc failed', e);
      }
      return {
        day: '',
        month: '',
    ...
    function generateCalendar() {
      ...
      while (cur.isSameOrBefore(endCal, 'day')) {
        ...
        dn.className = 'lunar-day-number';
        dn.textContent = cur.date();
        div.appendChild(dn);
        const ld = document.createElement('div');
        ld.className = 'lunar-lunar-day';
        ld.textContent = getLunarDate(cur).day;
        div.appendChild(ld);
        div.addEventListener('click', function () {
          if (div.classList.contains('other-month')) return;
          state.selectedDate = cur.clone();
          updateHeader();
          generateCalendar();
        });
    ...
    function bindEvents() {
      ...
      el.viewBtn && el.viewBtn.addEventListener('click', () => {
        const m = parseInt(el.monthSelector.value, 10);
        const y = parseInt(el.yearSelector.value, 10);
        state.currentDate = moment([y, m - 1]);
        updateHeader();
        generateCalendar();
      });
